...
1. Создание и подготовка документа
   ...
   - Редактирование онлайн (текст, презентации)
   - Сохранение в облаке
   - Автосохранение версионирование
...
2.2.2 Оценка модифицируемости

1. Модифицируемость анализируют по:
   - Границам ответственности (слабо или сильно связаны блоки).
   - Влиянию изменений (локализованность или каскадность).
   - Типичным сценариям изменений (что будет, если бизнесу нужно расширить систему).

2. Оценка по блокам
   1. Создание и подготовка документа 
      - Изменения: новые типы редакторов (например, таблицы), новые шаблоны. 
      - Оценка: высокая модифицируемость, если редактор и шаблоны сделаны как отдельные плагины/модули. 
      - Риск: если редактор tightly coupled с хранением, то любое обновление редактора может затронуть версионирование.
   2. Управление и хранение
      - Изменения: новые политики хранения (например, GDPR, срок хранения), добавление шеринга.
      - Оценка: средняя модифицируемость. Хранение тесно связано и с созданием, и с печатью. Изменения могут затронуть весь цикл.
      - Риск: монолитное хранилище → сложно масштабировать и менять без каскадных эффектов.
   3. Подготовка к печати
      - Изменения: новые параметры печати (цветность, тип бумаги), интеграция с другими сервисами (например, фотоцентр).
      - Оценка: высокая модифицируемость, если интерфейс печати проектируется через API и конфигурации.
      - Риск: tight coupling с оплатой и магазинами → может вызвать каскадные изменения.

   4. Оплата
      - Изменения: добавление новых методов (PayPal, крипта, BNPL), изменение правил биллинга.
      - Оценка: высокая модифицируемость при условии использования payment gateway abstraction (например, Stripe API).
      - Риск: если оплата встроена в сам заказ, а не вынесена отдельно, изменения будут затрагивать и подготовку к печати, и выполнение.

   5. Выполнение заказа
      - Изменения: новые типы магазинов (сервис-киоски, удалённые точки), разные очереди печати.
      - Оценка: средняя модифицируемость. Сильно зависит от интеграции с физическим оборудованием.
      - Риск: vendor lock-in (например, API печатных машин разных производителей несовместимы).

   6. Пост-обслуживание
      - Изменения: новые каналы обратной связи, программа лояльности, повторные заказы.
      - Оценка: высокая модифицируемость, так как этот блок слабо связан с остальными.
      - Риск: низкий, обычно расширения делаются ad-hoc.

3. Итоговая оценка модифицируемости (по шкале от 1 до 5)
   
   | Блок                  | Локализованность изменений | Зависимости | Оценка |
   | --------------------- | -------------------------- | ----------- | ------ |
   | Создание и подготовка | Высокая                    | Средние     | ⭐⭐⭐⭐☆  |
   | Управление и хранение | Средняя                    | Высокие     | ⭐⭐⭐☆☆  |
   | Подготовка к печати   | Высокая                    | Средние     | ⭐⭐⭐⭐☆  |
   | Оплата                | Высокая                    | Низкие      | ⭐⭐⭐⭐⭐  |
   | Выполнение заказа     | Средняя                    | Высокие     | ⭐⭐⭐☆☆  |
   | Пост-обслуживание     | Высокая                    | Низкие      | ⭐⭐⭐⭐⭐  |

4. Общий вывод:
   - Наиболее гибкие блоки: Оплата, Пост-обслуживание, Создание/печать.
   - Наименее модифицируемые: Хранение и Выполнение заказа (сильно связаны и зависят от инфраструктуры).
